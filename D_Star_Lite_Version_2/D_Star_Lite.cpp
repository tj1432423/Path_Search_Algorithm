...
void D_STAR_LITE::compute_shortest_path(){
    ...
    while(open_list.Heap_size() && (open_list.Heap_top().first < calculate_key(X_c) || abs(X_c->rhs - X_c->g) > epsion)){
        ...
        VALUE k_old=tmp.first;
        D_STAR_LITE_NODE* u=tmp.second;
        u->nodetype=D_STAR_LITE_NODE::Close;

        if(k_old < calculate_key(u)){
            open_list.Heap_push(make_pair(calculate_key(u),u));
            u->nodetype=D_STAR_LITE_NODE::Open;
        }
        else{
            if(u->g > u->rhs){
                u->g = u->rhs;
...
float D_STAR_LITE::calculate_h(const D_STAR_LITE_NODE* const _u){
    ...
    return float(0.99)*calculate_dis(_u,X_c);
    //return 0;
}

float D_STAR_LITE::calculate_dis(const D_STAR_LITE_NODE* const _x,const D_STAR_LITE_NODE* const _y){
    double dis_pow_2=pow((double(_x->Location.first-_y->Location.first)),2)+pow((double(_x->Location.second-_y->Location.second)),2);
