...
void LNode_Min_Heap_Opt::Heap_push(const pair<float,PNode *>& new_node_pair){
    Min_Heap.push_back(new_node_pair);
    Mp[new_node_pair.second]=Min_Heap.size()-1;
    siftup(Min_Heap.size()-1);
}

void LNode_Min_Heap_Opt::Heap_pop(){
    if (Min_Heap.empty()){
...
void LNode_Min_Heap_Opt::Heap_delect(PNode* obj_Node){
...
pair<float,PNode *> LNode_Min_Heap_Opt::Heap_top(){
    if (Min_Heap.empty()){
        cout<<" The Open_List_Min_Heap is empty !!! "<<endl;
...
size_t LNode_Min_Heap_Opt::Heap_size(){
    return Min_Heap.size();
}


void LNode_Min_Heap_Opt::siftdown(size_t index){     //将索引为Index的节点下沉
...
void LNode_Min_Heap_Opt::siftup(size_t index){     //将索引为Index的节点上浮，
    if(index>=Min_Heap.size()){
